...
function compareKeysNonEmpty(keys: SortKeys, a: number, b: number): number {
...
export function sortRows<T>(rows: T[], sorts: SortRule[], columns: ColumnDef<T>[]): T[] {
  ...
  order.sort((ia, ib) => {
    for (const criterion of criteria) {
      let result: number
      if (criterion.keys) {
        // Vacíos SIEMPRE al final, independientemente de la dirección:
        // se comparan fuera de la multiplicación por el signo.
        const aEmpty = criterion.keys.empty[ia]
        const bEmpty = criterion.keys.empty[ib]
        if (aEmpty || bEmpty) {
          if (aEmpty && bEmpty) continue
          return aEmpty ? 1 : -1
        }
        result = compareKeysNonEmpty(criterion.keys, ia, ib)
      } else {
        result = criterion.sortFn!(rows[ia], rows[ib])
      }
