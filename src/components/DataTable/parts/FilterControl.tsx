...
export function FilterControl<T>({ column, rule, setRule, labels, facets, debounceMs }: Props<T>) {
...
function TextFilter<T>({
  ...
  setRule,
  debounceMs,
}: Pick<Props<T>, 'column' | 'rule' | 'setRule' | 'debounceMs'>) {
...
function BooleanFilter<T>({
  ...
  setRule,
  labels,
}: Pick<Props<T>, 'column' | 'rule' | 'setRule' | 'labels'>) {
...
function SelectFilter<T>({
  ...
  labels,
  facets,
}: Pick<Props<T>, 'column' | 'rule' | 'setRule' | 'labels' | 'facets'>) {
